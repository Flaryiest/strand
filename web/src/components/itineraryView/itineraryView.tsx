...
import { ItineraryRecommendation, PlaceRecommendation } from '@/types/recommendation.types';
...
export default function ItineraryView({
  ...
  const handleSelectionChange = useCallback((slotId: string, selectedIndex: number) => {
    setSelections(prev => ({
      ...prev,
    ...
    return (
      <div className={styles.container}>
        <div className={styles.empty}>
          ...
          <p className={styles.emptyText}>No recommendations found. Try a different search.</p>
        </div>
      </div>
    );
  }
          ...
          <div className={styles.metaBar}>
            {itinerary.totalEstimatedTime && (
              <div className={styles.metaItem}>
                <svg className={styles.metaIcon} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <circle cx="12" cy="12" r="10" />
                  <polyline points="12 6 12 12 16 14" />
                </svg>
                <span>Duration:</span>
                <span className={styles.metaValue}>{itinerary.totalEstimatedTime}</span>
              </div>
            )}
            {itinerary.totalEstimatedCost && (
              <div className={styles.metaItem}>
                ...
                <span>Est. Cost:</span>
                <span className={styles.metaValue}>{itinerary.totalEstimatedCost}</span>
              </div>
            )}
            <div className={styles.metaItem}>
              <svg className={styles.metaIcon} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            {index < itinerary.slots.length - 1 && (
              <div className={styles.travelInfo}>
                <div className={styles.travelBadge}>
                  <svg className={styles.travelIcon} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <circle cx="12" cy="12" r="10" />
                    <polyline points="12 6 12 12 16 14" />
                  </svg>
                  ~15 min drive
